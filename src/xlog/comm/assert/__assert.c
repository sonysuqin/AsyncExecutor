...
#ifdef WIN32
#define __assert(__Expression, __FILE, __LINE) \
  (void)((assert(__Expression), 0))
#endif
...
EXPORT_FUNC void __ASSERT(const char* _pfile,
                          int _line,
                          const char* _pfunc,
...
void __ASSERTV2(const char* _pfile,
  ...
  offset +=
      snprintf(assertlog, sizeof(assertlog), "[ASSERT(%s)]", _pexpression);
  offset +=
      vsnprintf(assertlog + offset, sizeof(assertlog) - offset, _format, _list);
...
void __ASSERT2(const char* _pfile,
               int _line,
               const char* _pfunc,
               const char* _pexpression,
               const char* _format,
