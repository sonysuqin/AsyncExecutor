...
size_t AutoBuffer::Read(const off_t& _pos,
                        AutoBuffer& _rhs,
                        size_t _len) const {
  size_t readlen = Length() - _pos;
  readlen = min(readlen, _len);
...
off_t AutoBuffer::Move(off_t _move_len) {
  if (0 < _move_len) {
    ...
    Length(move_len < (size_t)Pos() ? Pos() - move_len : 0,
           Length() - move_len);
  }
...
void AutoBuffer::__FitSize(size_t _len) {
  if (_len > capacity_) {
    ...
    if (NULL == p) {
      ASSERT2(p,
              "_len=%" PRIu64 ", m_nMallocUnitSize=%" PRIu64
              ", nMallocSize=%" PRIu64 ", m_nCapacity=%" PRIu64,
              (uint64_t)_len, (uint64_t)malloc_unitsize_, (uint64_t)mallocsize,
              (uint64_t)capacity_);
