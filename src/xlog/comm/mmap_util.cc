...
bool OpenMmapFile(const char* _filepath,
  ...
  if (!file_exist && is_open) {
    // Extending a file with ftruncate, thus creating a big hole, and then
    // filling the hole by mod-ifying a shared mmap() can lead to SIGBUS when no
    // space left the boost library uses ftruncate, so we pre-allocate the file's
