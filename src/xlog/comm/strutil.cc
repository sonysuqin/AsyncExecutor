...
/*#define EQUALSIGNORECASE(T) bool EqualsIgnoreCase(const T& str1, const T&
str2)\
{\
    T str1temp = str1;\
    T str2temp = str2;\
...
#define SPLITTOKEN(T)                                           \
  std::vector<T>& SplitToken(const T& str, const T& delimiters, \
                             std::vector<T>& ss) {              \
...
std::string ReplaceChar(const char* const input_str,
                        char be_replaced,
                        char replace_with) {
...
size_t ci_find_substr(const std::string& str,
                      const std::string& sub,
                      size_t pos) {
  const std::locale& loc = std::locale();
  typename std::string::const_iterator it =
      std::search(str.begin() + pos, str.end(), sub.begin(), sub.end(),
