...
uint64_t gettickcount() {
  ...
  struct timespec ts;
  int result =
      ioctl(s_fd, ANDROID_ALARM_GET_TIME(ANDROID_ALARM_ELAPSED_REALTIME), &ts);
...
#if 0  // defined(TARGET_OS_IPHONE) || defined(TARGET_IPHONE_SIMULATOR)
       // //gettimeofday will rollback, KERN_BOOTTIME up late
#include <assert.h>
...
uint64_t gettickcount() {
  ...
  // Convert to nanoseconds - if this is the first time we've run, get the
  // timebase.
  ...
  uint64_t millis =
      (mach_time * timebase_info.numer) / (timebase_info.denom * 1000000);
  return millis;
}
#endif
