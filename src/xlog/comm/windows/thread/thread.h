...
#include "xlog/comm/thread/runnable.h"
#include "xlog/comm/windows/thread/lock.h"
...
class ThreadUtil {
 public:
  ...
  static void usleep(unsigned int _usec) {
    std::this_thread::sleep_for(std::chrono::microseconds(_usec));
  }
  ...
  static thread_tid currentthreadid() {
    ...
    return (thread_tid)std::stoul(stid);
  }
  ...
  static int createThread(thread_handler& pth,
                          THREAD_START_PROC proc,
                          void* args) {
    if (pth != NULL) {
      delete pth;
    }
...
class Thread {
 ...
 public:
  ...
  explicit Thread(const T& op,
                  const char* _thread_name = NULL,
                  bool _outside_join = false)
  ...
  int start(bool* _newone = NULL) {
    ...
    // int ret = thrd_create(&m_runableref->tid, (thrd_start_t)&start_routine,
    // (void*)m_runableref);
    int ret = ThreadUtil::createThread(m_runableref->m_th, &start_routine,
                                       (void*)m_runableref);
  ...
  int start_after(unsigned int after) {
    ...
    // int ret = thrd_create(&m_runableref->tid,
    // (thrd_start_t)&start_routine_after, (void*)m_runableref);
    int ret = ThreadUtil::createThread(m_runableref->m_th, &start_routine_after,
                                       (void*)m_runableref);
    ASSERT(thrd_success == ret);
  ...
  int start_periodic(unsigned int after, unsigned int periodic) {  // ms
    ...
    // int ret = thrd_create(&m_runableref->tid,
    // (thrd_start_t)&start_routine_periodic, (void*)m_runableref);
    int ret = ThreadUtil::createThread(
        m_runableref->m_th, &start_routine_periodic, (void*)m_runableref);
    ASSERT(thrd_success == ret);
  ...
  bool isruning() const { return !m_runableref->isended; }

 private:
  static void init(void* arg) {
    volatile RunnableReference* runableref =
        static_cast<RunnableReference*>(arg);
  ...
  static void* start_routine_after(void* arg) {
    ...
    if (!runableref->iscanceldelaystart) {
      (const_cast<RunnableReference*>(runableref))
          ->condtime.wait(runableref->aftertime);
  ...
  static void* start_routine_periodic(void* arg) {
    ...
    if (!runableref->iscanceldelaystart) {
      ...
      while (!runableref->iscanceldelaystart) {
        ...
        if (!runableref->iscanceldelaystart)
          (const_cast<RunnableReference*>(runableref))
              ->condtime.wait(runableref->periodictime);
      }
    }
