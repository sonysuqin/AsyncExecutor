...
class Tss {
 public:
  ...
  ~Tss() { TlsFree(m_dwTlsIndex); }

  void* get() const { return TlsGetValue(m_dwTlsIndex); }
...
class Tss {
 public:
  ...
  ~Tss() { tss_delete(m_key); }

  void* get() const { return tss_get(m_key); }

  void set(void* value) { tss_set(m_key, value); }
