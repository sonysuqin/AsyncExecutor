...
char* w2c(char* pcstr, size_t len, const wchar_t* pwstr) {
  ...
  int nbytes = WideCharToMultiByte(
      ...
      NULL,  // no output buffer given, we just want to know how long it needs
             // to be
      0,
      NULL,   // no replacement character given
      NULL);  // we don't want to know if a character didn't make it through the
              // translation
...
bool getProxyAddr(const std::string& strAddr,
                  char* strDestAddr,
                  const char* type) {
...
int getIEProxy(const wchar_t* host,
               int& proxytype,
               int& port,
               char* strAddr,
               bool bUserHttps) {
  ...
  if (fAutoProxy) {
    ...
    fAutoProxy =
        WinHttpGetProxyForUrl(session, host, &autoProxyOptions, &autoProxyInfo);
  ...
  if (fAutoProxy) {
    ...
    if (ieProxyConfig.lpszProxy != NULL) {
      ...
      /// may be like this:
      /// "http=127.0.0.1:8888;https=127.0.0.1:8888;ftp=127.0.0.1:8888;socks=127.0.0.1:8888"
      /// "127.0.0.1:8888"
  ...
  // if(autoProxyOptions.lpszAutoConfigUrl != NULL)
  // GlobalFree(autoProxyOptions.lpszAutoConfigUrl);
  if (ieProxyConfig.lpszAutoConfigUrl != NULL)
...
bool getProxyInfoImpl(int& port,
                      std::string& strProxy,
