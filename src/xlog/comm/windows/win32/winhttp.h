...
#define WINHTTP_FLAG_ASYNC \
  0x10000000  // this session is asynchronous (where supported)

// flags for WinHttpOpenRequest():
#define WINHTTP_FLAG_SECURE 0x00800000  // use SSL if applicable (HTTPS)
#define WINHTTP_FLAG_ESCAPE_PERCENT \
  0x00000004  // if escaping enabled, escape percent as well
#define WINHTTP_FLAG_NULL_CODEPAGE \
  0x00000008  // assume all symbols are ASCII, use fast convertion
#define WINHTTP_FLAG_BYPASS_PROXY_CACHE \
  0x00000100  // add "pragma: no-cache" request header
#define WINHTTP_FLAG_REFRESH WINHTTP_FLAG_BYPASS_PROXY_CACHE
#define WINHTTP_FLAG_ESCAPE_DISABLE 0x00000040  // disable escaping
#define WINHTTP_FLAG_ESCAPE_DISABLE_QUERY \
  0x00000080  // if escaping enabled escape path part, but do not escape query
...
// the callback with WINHTTP_CALLBACK_STATUS_REQUEST_COMPLETE. It is not
// sufficient to just return the result of the async operation. If the API
// failed then the app cannot call GetLastError() because the thread context
// will be incorrect. Both the value returned by the async API and any resultant
// error code are made available. The app need not check dwError if dwResult
// indicates that the API succeeded (in this case dwError will be ERROR_SUCCESS)
//
...
// typedef int INTERNET_SCHEME, * LPINTERNET_SCHEME;   // duplicate with
// wininet.h
...
typedef struct        // duplicate with wininet.h
{
    ...
    LPWSTR  lpszExtraInfo;      // pointer to extra information (e.g. ?foo or
#foo) DWORD   dwExtraInfoLength;  // length of extra information
}
...
WinHttpTimeFromSystemTime(__in CONST SYSTEMTIME* pst,  // input GMT time
                          __out_bcount(WINHTTP_TIME_FORMAT_BUFSIZE)
...
#define ICU_NO_ENCODE \
  0x20000000  // Don't convert unsafe characters to escape sequence
...
#define ICU_ESCAPE_AUTHORITY \
  0x00002000  // causes InternetCreateUrlA to escape chars in authority
              // components (user, pwd, host)
#define ICU_REJECT_USERPWD \
  0x00004000  // rejects usrls whick have username/pwd sections

BOOLAPI
WinHttpCheckPlatform(void);

WINHTTPAPI BOOL WINAPI
...
BOOLAPI
WinHttpSetOption(IN HINTERNET hInternet,
                 IN DWORD dwOption,
                 IN LPVOID lpBuffer,
...
#define WINHTTP_AUTOLOGON_SECURITY_LEVEL_DEFAULT \
...
#define WINHTTP_OPTION_REDIRECT_POLICY_LAST \
...
#define WINHTTP_CALLBACK_FLAG_DETECTING_PROXY \
...
#define WINHTTP_CALLBACK_FLAG_SECURE_FAILURE \
  WINHTTP_CALLBACK_STATUS_SECURE_FAILURE
#define WINHTTP_CALLBACK_FLAG_SENDREQUEST_COMPLETE \
  WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE
#define WINHTTP_CALLBACK_FLAG_HEADERS_AVAILABLE \
  WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE
#define WINHTTP_CALLBACK_FLAG_DATA_AVAILABLE \
  WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE
#define WINHTTP_CALLBACK_FLAG_READ_COMPLETE \
  WINHTTP_CALLBACK_STATUS_READ_COMPLETE
#define WINHTTP_CALLBACK_FLAG_WRITE_COMPLETE \
  WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE
#define WINHTTP_CALLBACK_FLAG_REQUEST_ERROR \
...
// parameter of WinHttpQueryHeaders() AND the header being queried contains date
// information, e.g. the "Expires:" header then lpBuffer will contain a
// SYSTEMTIME structure containing the date and time information converted from
// the header string
//
...
BOOLAPI
WinHttpAddRequestHeaders(IN HINTERNET hRequest,
...
//
// WINHTTP_ADDREQ_FLAG_ADD - if WINHTTP_ADDREQ_FLAG_REPLACE is set but the
// header is not found then if this flag is set, the header is added anyway, so
// long as there is a valid header-value
//
...
//
// WINHTTP_ADDREQ_FLAG_REPLACE - replaces the specified header. Only one header
// can be supplied in the buffer. If the header to be replaced is not the first
...
BOOLAPI WinHttpQueryAuthSchemes(
    IN HINTERNET hRequest,  // HINTERNET handle returned by WinHttpOpenRequest
    OUT LPDWORD
        lpdwSupportedSchemes,  // a bitmap of available Authentication Schemes
    OUT LPDWORD lpdwFirstScheme,  // returns the first auth scheme returned by
                                  // the server
...
BOOLAPI
WinHttpDetectAutoProxyConfigUrl(__in DWORD dwAutoDetectFlags,
...
#define ERROR_WINHTTP_CHUNKED_ENCODING_HEADER_SIZE_OVERFLOW \
...
#define ERROR_WINHTTP_CLIENT_CERT_NO_ACCESS_PRIVATE_KEY \
  (WINHTTP_ERROR_BASE + 186)
