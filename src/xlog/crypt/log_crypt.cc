...
LogCrypt::LogCrypt(const char* _pubkey) : seq_(0), is_crypt_(false) {
  ...
  if (0 ==
      uECC_make_key((uint8_t*)client_pubkey_, client_pri, uECC_secp256k1())) {
    return;
  }

  uint8_t ecdh_key[32] = {0};
  if (0 ==
      uECC_shared_secret(svr_pubkey, client_pri, ecdh_key, uECC_secp256k1())) {
    return;
  }
...
/*
 * |magic start(char)|seq(uint16_t)|begin hour(char)|end
 * hour(char)|length(uint32_t)|crypt key(char*64)|
 */

uint32_t LogCrypt::GetHeaderLen() {
  return sizeof(char) * 3 + sizeof(uint16_t) + sizeof(uint32_t) +
         sizeof(char) * 64;
}
...
bool LogCrypt::GetLogHour(const char* const _data,
                          size_t _len,
                          int& _begin_hour,
                          int& _end_hour) {
...
void LogCrypt::UpdateLogHour(char* _data) {
  ...
  memcpy(_data + GetHeaderLen() - sizeof(uint32_t) - sizeof(char) * 64 -
             sizeof(char),
         &hour, sizeof(hour));
}
...
uint32_t LogCrypt::GetLogLen(const char* const _data, size_t _len) {
  ...
  uint32_t len = 0;
  memcpy(&len, _data + GetHeaderLen() - sizeof(uint32_t) - sizeof(char) * 64,
         sizeof(len));
  return len;
}
...
void LogCrypt::SetHeaderInfo(char* _data, bool _is_async) {
  ...
  memcpy(_data + sizeof(kMagicAsyncStart) + sizeof(seq_) + sizeof(hour), &hour,
         sizeof(hour));

  uint32_t len = 0;
  memcpy(_data + sizeof(kMagicAsyncStart) + sizeof(seq_) + sizeof(hour) * 2,
         &len, sizeof(len));
  memcpy(_data + sizeof(kMagicAsyncStart) + sizeof(seq_) + sizeof(hour) * 2 +
             sizeof(len),
...
bool LogCrypt::GetPeriodLogs(const char* const _log_path,
                             int _begin_hour,
                             int _end_hour,
  ...
  if (NULL == _log_path || _end_hour <= _begin_hour) {
    snprintf(msg, sizeof(msg),
             "NULL == _logPath || _endHour <= _beginHour, %d, %d", _begin_hour,
             _end_hour);
    return false;
  }
  ...
  if (0 != fseek(file, 0, SEEK_END)) {
    snprintf(msg, sizeof(msg), "fseek(file, 0, SEEK_END):%s",
  ...
  if (0 != fseek(file, 0, SEEK_SET)) {
    snprintf(msg, sizeof(msg), "fseek(file, 0, SEEK_SET) error:%s",
             strerror(ferror(file)));
    _err_msg += msg;
    fclose(file);
    return false;
  ...
  while (!feof(file) && !ferror(file)) {
    if ((long)(ftell(file) + GetHeaderLen() + GetTailerLen()) > file_size) {
      snprintf(msg, sizeof(msg),
               "ftell(file) + __GetHeaderLen() + sizeof(kMagicEnd)) > "
               "file_size error");
      break;
    }
    ...
    if (GetHeaderLen() != fread(header_buff, 1, GetHeaderLen(), file)) {
      snprintf(msg, sizeof(msg),
               "fread(buff.Ptr(), 1, __GetHeaderLen(), file) error:%s, "
               "before_len:%ld.",
    ...
    if (!GetLogHour(header_buff, GetHeaderLen(), begin_hour, end_hour)) {
      snprintf(msg, sizeof(msg),
               "__GetLogHour(buff.Ptr(), buff.Length(), beginHour, endHour) "
               "err, before_len:%ld.",
               before_len);
      break;
    }
  ...
  if (!ret) {
    ...
    snprintf(msg, sizeof(msg), "begintpos:%lu, endpos:%lu, filesize:%ld.",
             _begin_pos, _end_pos, file_size);
    _err_msg += msg;
  }
...
void LogCrypt::CryptSyncLog(const char* const _log_data,
  ...
  /*   uint32_t tmp[2] = {0};
     ...
     for (size_t i = 0; i < cnt; ++i) {
         ...
         memcpy((char*)_out_buff.Ptr() + header_len + i * TEA_BLOCK_LEN, tmp,
     TEA_BLOCK_LEN);
     }

     memcpy((char*)_out_buff.Ptr() + header_len + _input_len - remain_len,
     _log_data + _input_len - remain_len, remain_len);*/
#endif
}
...
bool LogCrypt::Fix(char* _data,
                   size_t _data_len,
                   bool& _is_async,
                   uint32_t& _raw_log_len) {
