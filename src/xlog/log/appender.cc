...
#include "xlog/comm/thread/condition.h"
#include "xlog/comm/thread/lock.h"
#include "xlog/comm/thread/thread.h"

#include "xlog/comm/autobuffer.h"
#include "xlog/comm/bootrun.h"
#include "xlog/comm/mmap_util.h"
#include "xlog/comm/ptrbuffer.h"
#include "xlog/comm/scope_recursion_limit.h"
#include "xlog/comm/strutil.h"
#include "xlog/comm/tickcount.h"
#include "xlog/comm/time_utils.h"
#include "xlog/comm/verinfo.h"
...
extern void log_formater(const XLoggerInfo* _info,
                         const char* _logbody,
                         PtrBuffer& _log);
...
static Condition& sg_cond_buffer_async =
    *(new Condition());  // 改成引用, 避免在全局释放时执行析构导致crash
#else
static Condition sg_cond_buffer_async;
#endif
...
static boost::iostreams::mapped_file& sg_mmmap_file =
    *(new boost::iostreams::mapped_file);
...
static std::string __make_logfilenameprefix(const timeval& _tv,
                                            const char* _prefix) {
  time_t sec = _tv.tv_sec;
  tm tcur = *localtime((const time_t*)&sec);

  char temp[64] = {0};
  snprintf(temp, 64, "_%d%02d%02d", 1900 + tcur.tm_year, 1 + tcur.tm_mon,
           tcur.tm_mday);
...
static void __get_filenames_by_prefix(const std::string& _logdir,
  ...
  for (boost::filesystem::directory_iterator iter(path); iter != end_iter;
    ...
    if (boost::filesystem::is_regular_file(iter->status())) {
      ...
      if (strutil::StartsWith(filename, _fileprefix) &&
          strutil::EndsWith(filename, _fileext)) {
...
static void __get_filepaths_from_timeval(
    const timeval& _tv,
  ...
  for (std::vector<std::string>::iterator iter = filename_vec.begin();
       iter != filename_vec.end(); ++iter) {
    _filepath_vec.push_back(_logdir + "/" + (*iter));
  }
}

static bool __string_compare_greater(const std::string& s1,
                                     const std::string& s2) {
...
static long __get_next_fileindex(const std::string& _fileprefix,
                                 const std::string& _fileext) {
  std::vector<std::string> filename_vec;
  __get_filenames_by_prefix(sg_logdir, _fileprefix, _fileext, filename_vec);
  if (!sg_cache_logdir.empty()) {
    __get_filenames_by_prefix(sg_cache_logdir, _fileprefix, _fileext,
                              filename_vec);
  }
  ...
  if (index_len > 0) {
    std::string index_str =
        last_filename.substr(_fileprefix.length(), index_len);
  ...
  if (boost::filesystem::exists(path) &&
    ...
    for (boost::filesystem::directory_iterator iter(path); iter != end_iter;
      ...
      if (now_time > file_modify_time &&
          now_time - file_modify_time > sg_max_alive_time) {
        if (boost::filesystem::is_regular_file(iter->status()) &&
...
static bool __append_file(const std::string& _src_file,
                          const std::string& _dst_file) {
...
static void __move_old_files(const std::string& _src_path,
  ...
  for (boost::filesystem::directory_iterator iter(path); iter != end_iter;
    ...
    if (!__append_file(iter->path().string(),
                       sg_logdir + "/" + iter->path().filename().string())) {
      break;
    }
...
static bool __openlogfile(const std::string& _log_dir) {
  ...
  __make_logfilename(tv, _log_dir, sg_logfileprefix.c_str(), LOG_EXT,
                     logfilepath, 1024);
  ...
  if (now_time < s_last_time) {
    ...
    if (NULL == sg_logfile) {
      __writetips2console("open file error:%d %s, path:%s", errno,
                          strerror(errno), s_last_file_path);
    }
  ...
  if (0 != s_last_time && (now_time - s_last_time) >
                              (time_t)((now_tick - s_last_tick) / 1000 + 300)) {
    struct tm tm_tmp = *localtime((const time_t*)&s_last_time);
    char last_time_str[64] = {0};
    strftime(last_time_str, sizeof(last_time_str), "%Y-%m-%d %z %H:%M:%S",
             &tm_tmp);

    tm_tmp = *localtime((const time_t*)&now_time);
    char now_time_str[64] = {0};
    strftime(now_time_str, sizeof(now_time_str), "%Y-%m-%d %z %H:%M:%S",
             &tm_tmp);

    char log[1024] = {0};
    snprintf(log, sizeof(log),
             "[F][ last log file:%s from %s to %s, time_diff:%lld, "
             "tick_diff:%" PRIu64 "\n",
             s_last_file_path, last_time_str, now_time_str,
             now_time - s_last_time, now_tick - s_last_tick);
...
static bool __cache_logs() {
  ...
  gettimeofday(&tv, NULL);
  char logfilepath[1024] = {0};
  __make_logfilename(tv, sg_logdir, sg_logfileprefix.c_str(), LOG_EXT,
                     logfilepath, 1024);
  if (boost::filesystem::exists(logfilepath)) {
    return false;
  }

  static const uintmax_t kAvailableSizeThreshold =
      (uintmax_t)1 * 1024 * 1024 * 1024;  // 1G
  boost::filesystem::space_info info =
      boost::filesystem::space(sg_cache_logdir);
...
static void __log2file(const void* _data, size_t _len, bool _move_file) {
  ...
  __make_logfilename(tv, sg_cache_logdir, sg_logfileprefix.c_str(), LOG_EXT,
                     logcachefilepath, 1024);

  bool cache_logs = __cache_logs();
  if ((cache_logs || boost::filesystem::exists(logcachefilepath)) &&
...
static void __writetips2file(const char* _tips_format, ...) {
  ...
  AutoBuffer tmp_buff;
  sg_log_buff->Write(tips_info, strnlen(tips_info, sizeof(tips_info)),
                     tmp_buff);
...
void xlogger_appender(const XLoggerInfo* _info, const char* _log) {
  ...
  if (2 <= (int)recursion.Get() && NULL == s_recursion_str.get()) {
    ...
    snprintf(recursive_log, sizeof(recursive_log),
             "ERROR!!! xlogger_appender Recursive calls!!!, count:%d",
             (int)recursion.Get());
...
static unsigned int to_string(const void* signature, int len, char* str) {
  ...
  for (sig_p = (const unsigned char*)signature;
       sig_p - (const unsigned char*)signature < len; sig_p++) {
    char high, low;
    high = *sig_p / 16;
    low = *sig_p % 16;
...
const char* xlogger_dump(const void* _dumpbuffer, size_t _len) {
  ...
  snprintf(forder_name, sizeof(forder_name), "%d%02d%02d", 1900 + tcur.tm_year,
           1 + tcur.tm_mon, tcur.tm_mday);
  ...
  snprintf(file_name, sizeof(file_name), "%d%02d%02d%02d%02d%02d_%d.dump",
           1900 + tcur.tm_year, 1 + tcur.tm_mon, tcur.tm_mday, tcur.tm_hour,
           tcur.tm_min, tcur.tm_sec, (int)_len);
  filepath += file_name;

  FILE* fileid = fopen(filepath.c_str(), "wb");

  if (NULL == fileid) {
    ASSERT2(NULL != fileid, "%s, errno:(%d, %s)", filepath.c_str(), errno,
            strerror(errno));
    return "";
  }
  ...
  dump_log +=
      snprintf(dump_log, 4096, "\n dump file to %s :\n", filepath.c_str());

  int dump_len = 0;

  for (int x = 0; x < 32 && dump_len < (int)_len; ++x) {
    dump_log += to_string((const char*)_dumpbuffer + dump_len,
                          std::min(int(_len) - dump_len, 16), dump_log);
...
static void get_mark_info(char* _info, size_t _infoLen) {
  ...
  snprintf(_info, _infoLen, "[%" PRIdMAX ",%" PRIdMAX "][%s]", xlogger_pid(),
           xlogger_tid(), tmp_time);
}
...
void appender_open(TAppenderMode _mode,
  ...
  if (!sg_log_close) {
    __writetips2file("appender has already been opened. _dir:%s _nameprefix:%s",
                     _dir, _nameprefix);
    return;
  }
  ...
  tickcount_t tick;
  tick.gettickcount();
  Thread(std::bind(&__del_timeout_file, _dir)).start_after(2 * 60 * 1000);
  ...
  if (OpenMmapFile(mmap_file_path, kBufferBlockLength, sg_mmmap_file)) {
    sg_log_buff =
        new LogBuffer(sg_mmmap_file.data(), kBufferBlockLength, true, _pub_key);
    use_mmap = true;
  } else {
  ...
  snprintf(appender_info, sizeof(appender_info),
           "^^^^^^^^^^" __DATE__ "^^^" __TIME__ "^^^^^^^^^^%s", mark_info);

  xlogger_appender(NULL, appender_info);
  char logmsg[256] = {0};
  snprintf(logmsg, sizeof(logmsg), "get mmap time: %" PRIu64,
           (int64_t)get_mmap_time);
  ...
  snprintf(logmsg, sizeof(logmsg), "log appender mode:%d, use mmap:%d",
           (int)_mode, use_mmap);
  xlogger_appender(NULL, logmsg);
  ...
  if (!sg_cache_logdir.empty()) {
    ...
    snprintf(logmsg, sizeof(logmsg),
             "cache dir space info, capacity:%" PRIuMAX " free:%" PRIuMAX
             " available:%" PRIuMAX,
  ...
  snprintf(logmsg, sizeof(logmsg),
           "log dir space info, capacity:%" PRIuMAX " free:%" PRIuMAX
           " available:%" PRIuMAX,
           info.capacity, info.free, info.available);
  xlogger_appender(NULL, logmsg);
...
void appender_open_with_cache(TAppenderMode _mode,
  ...
  if (!_cachedir.empty()) {
    ...
    Thread(std::bind(&__del_timeout_file, _cachedir))
        .start_after(2 * 60 * 1000);
    // "_nameprefix" must explicitly convert to "std::string", or when the
    // thread is ready to run, "_nameprefix" has been released.
    Thread(std::bind(&__move_old_files, _cachedir, _logdir,
...
bool appender_getfilepath_from_timespan(
    int _timespan,
    const char* _prefix,
    std::vector<std::string>& _filepath_vec) {
  if (sg_logdir.empty())
  ...
  if (!sg_cache_logdir.empty()) {
    __get_filepaths_from_timeval(tv, sg_cache_logdir, _prefix, LOG_EXT,
                                 _filepath_vec);
  }
  return true;
}

bool appender_make_logfile_name(int _timespan,
                                const char* _prefix,
                                std::vector<std::string>& _filepath_vec) {
  if (sg_logdir.empty())
    return false;
  ...
  __make_logfilename(tv, sg_logdir, _prefix, LOG_EXT, log_path,
                     sizeof(log_path));
  ...
  __make_logfilename(tv, sg_cache_logdir, _prefix, LOG_EXT, cache_log_path,
                     sizeof(cache_log_path));
  ...
  if (!boost::filesystem::exists(log_path) &&
      !boost::filesystem::exists(cache_log_path)) {
    _filepath_vec.push_back(log_path);
  }
