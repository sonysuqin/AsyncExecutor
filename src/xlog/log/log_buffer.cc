...
bool LogBuffer::GetPeriodLogs(const char* _log_path,
                              int _begin_hour,
                              int _end_hour,
                              unsigned long& _begin_pos,
                              unsigned long& _end_pos,
                              std::string& _err_msg) {
  return LogCrypt::GetPeriodLogs(_log_path, _begin_hour, _end_hour, _begin_pos,
                                 _end_pos, _err_msg);
}

LogBuffer::LogBuffer(void* _pbuffer,
                     size_t _len,
                     bool _isCompress,
                     const char* _pubkey)
    : is_compress_(_isCompress),
      log_crypt_(new LogCrypt(_pubkey)),
...
bool LogBuffer::Write(const void* _data,
                      size_t _inputlen,
...
bool LogBuffer::Write(const void* _data, size_t _length) {
  ...
  log_crypt_->CryptAsyncLog((char*)buff_.Ptr() + before_len,
                            write_len + remain_nocrypt_len_, out_buffer,
                            remain_nocrypt_len_);
...
bool LogBuffer::__Reset() {
  ...
  if (is_compress_) {
    ...
    if (Z_OK != deflateInit2(&cstream_, Z_BEST_COMPRESSION, Z_DEFLATED,
...
void LogBuffer::__Flush() {
  ...
  buff_.Length(buff_.Length() + log_crypt_->GetTailerLen(),
...
void LogBuffer::__Fix() {
  ...
  if (log_crypt_->Fix((char*)buff_.Ptr(), buff_.Length(), is_compress,
                      raw_log_len)) {
    buff_.Length(raw_log_len + log_crypt_->GetHeaderLen(),
