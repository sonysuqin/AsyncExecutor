...
#include "xlog/comm/string_cast.h"
...
#define XLOGGER_STATIC_ASSERT(...)                                      \
  typedef ::xlogger_static_assert_test<sizeof(                          \
      ::XLOGGER_STATIC_ASSERTION_FAILURE<((__VA_ARGS__) == 0 ? false    \
                                                             : true)>)> \
...
#define XLOGGER_FORMAT_ARGS(n) PP_ENUM_TRAILING_PARAMS(n, const string_cast& a)
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(0));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(1));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(2));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(3));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(4));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(5));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(6));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(7));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(8));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(9));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(10));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(11));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(12));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(13));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(14));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(15));
  XMessage& operator()(const TypeSafeFormat&,
                       const char* _format XLOGGER_FORMAT_ARGS(16));
...
class XLogger {
 public:
  ...
  ~XLogger() {
    ...
    gettimeofday((struct timeval*)&m_info.timeval, NULL);
 ...
 public:
  ...
  void operator<<(XLogger& _xlogger) { _xlogger.operator>>(*this); }
...
#define XLOGGER_FORMAT_ARGS(n) PP_ENUM_TRAILING_PARAMS(n, const string_cast& a)
  XLogger& operator()(const TypeSafeFormat&,
                      const char* _format XLOGGER_FORMAT_ARGS(0));
  XLogger& operator()(const TypeSafeFormat&,
                      const char* _format XLOGGER_FORMAT_ARGS(1));
  XLogger& operator()(const TypeSafeFormat&,
 ...
 public:
  XScopeTracer(TLogLevel _level,
    ...
    if (m_enable) {
      ...
      m_tv.tv_sec = m_info.timeval.tv_sec;
      m_tv.tv_usec = m_info.timeval.tv_usec;
      char strout[1024] = {'\0'};
      snprintf(strout, sizeof(strout), "-> %s %s", m_name,
               NULL != _log ? _log : "");
  ...
  ~XScopeTracer() {
    if (m_enable) {
      ...
      m_info.timeval.tv_sec = tv.tv_sec;
      m_info.timeval.tv_usec = tv.tv_usec;
      long timeSpan =
          (tv.tv_sec - m_tv.tv_sec) * 1000 + (tv.tv_usec - m_tv.tv_usec) / 1000;
      char strout[1024] = {'\0'};
      snprintf(strout, sizeof(strout), "<- %s +%ld, %s", m_name, timeSpan,
               m_exitmsg.c_str());
      xlogger_Write(&m_info, strout);
    }
  }
...
#define XLOGGER_TYPESAFE_FORMAT_IMPLEMENT(n, m)                            \
  inline XMessage& XMessage::operator()(                                   \
      const TypeSafeFormat&, const char* _format XLOGGER_FORMAT_ARGS(n)) { \
    if (_format != NULL) {                                                 \
      const string_cast* args[16] = {XLOGGER_VARIANT_ARGS(n) PP_COMMA_IF(  \
          PP_AND(n, m)) XLOGGER_VARIANT_ARGS_NULL(m)};                     \
...
inline void XMessage::DoTypeSafeFormat(const char* _format,
                                       const string_cast** _args) {
...
inline XLogger& XLogger::operator>>(const string_cast& _value) {
  if (NULL != _value.str()) {
    ...
    m_info.level = kLevelFatal;
    m_message.insert(0,
                     "{!!! XLogger& XLogger::operator>>(const string_cast& "
                     "_value): _value.str() == NULL !!!}");
    assert(false);
  }
  return *this;
}
...
inline XLogger& XLogger::VPrintf(const char* _format, va_list _list) {
  if (_format == NULL) {
    m_info.level = kLevelFatal;
    m_message +=
        "{!!! XLogger& XLogger::operator()(const char* _format, va_list "
        "_list): _format == NULL !!!}";
    assert(false);
    return *this;
  }
...
inline XLogger& XLogger::operator()(const char* _format, ...) {
  if (_format == NULL) {
    m_info.level = kLevelFatal;
    m_message +=
        "{!!! XLogger& XLogger::operator()(const char* _format, ...): _format "
        "== NULL !!!}";
    assert(false);
    return *this;
  }
...
#define XLOGGER_VARIANT_ARGS_NULL(n) PP_ENUM(n, NULL)
#define XLOGGER_TYPESAFE_FORMAT_IMPLEMENT(n, m)                            \
  inline XLogger& XLogger::operator()(                                     \
      const TypeSafeFormat&, const char* _format XLOGGER_FORMAT_ARGS(n)) { \
    if (_format != NULL) {                                                 \
      const string_cast* args[16] = {XLOGGER_VARIANT_ARGS(n) PP_COMMA_IF(  \
          PP_AND(n, m)) XLOGGER_VARIANT_ARGS_NULL(m)};                     \
...
inline void XLogger::DoTypeSafeFormat(const char* _format,
                                      const string_cast** _args) {
  const char* current = _format;
  int count = 0;
  while ('\0' != *current) {
...
#define XLOGGER_ROUTER_OUTPUT(op1, op, ...) \
  PP_IF(PP_NUM_PARAMS(__VA_ARGS__),         \
        PP_IF(PP_DEC(PP_NUM_PARAMS(__VA_ARGS__)), op, op1), )
...
#define __xlogger_c_impl(level, ...) \
  xlogger2(level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, __VA_ARGS__)
#define __xlogger_c_impl_if(level, exp, ...)                               \
  xlogger2_if(exp, level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, \
              __VA_ARGS__)
...
#define xverbose2_if(exp, ...) \
  __xlogger_c_impl_if(kLevelVerbose, exp, __VA_ARGS__)
...
#define xassert2(exp, ...)                                                  \
  if (((exp) || !xlogger_IsEnabledFor(kLevelFatal)))                        \
    ...
    XLoggerInfo info = {kLevelFatal, XLOGGER_TAG, __XFILE__, __XFUNCTION__, \
                        __LINE__,    {0, 0},      -1,        -1,            \
                        -1};                                                \
    gettimeofday(&info.m_tv, NULL);                                         \
...
#define xlogger(level, tag, file, func, line, ...)             \
  if ((!xlogger_IsEnabledFor(level)))                          \
    ...
    XLogger(level, tag, file, func, line, XLOGGER_HOOK)        \
        XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(TSF __VA_ARGS__), \
                              (TSF __VA_ARGS__), __VA_ARGS__)
...
#define xlogger2(level, tag, file, func, line, ...)                       \
  if ((!xlogger_IsEnabledFor(level)))                                     \
    ...
    XLogger(level, tag, file, func, line, XLOGGER_HOOK)                   \
        XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__), (__VA_ARGS__), \
                              __VA_ARGS__)
...
#define __xlogger_cpp_impl2(level, ...) \
  xlogger2(level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, __VA_ARGS__)
...
#define xverbose2_if(exp, ...) \
  __xlogger_cpp_impl_if(kLevelVerbose, exp, __VA_ARGS__)
#define xdebug2_if(exp, ...) \
  __xlogger_cpp_impl_if(kLevelDebug, exp, __VA_ARGS__)
...
#define xerror2_if(exp, ...) \
  __xlogger_cpp_impl_if(kLevelError, exp, __VA_ARGS__)
#define xfatal2_if(exp, ...) \
  __xlogger_cpp_impl_if(kLevelFatal, exp, __VA_ARGS__)
#define xlog2_if(level, ...) __xlogger_cpp_impl_if(level, __VA_ARGS__)

#define xgroup2_define(group)                                               \
  XLogger group(kLevelAll, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, \
                XLOGGER_HOOK)
#define xgroup2(...)                                                  \
  XLogger(kLevelAll, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, \
          XLOGGER_HOOK)(__VA_ARGS__)
...
#define xassert2(exp, ...)                                                \
  if (((exp) || !xlogger_IsEnabledFor(kLevelFatal)))                      \
    ...
    XLogger(kLevelFatal, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, \
            XLOGGER_HOOK)                                                 \
        .Assert(#exp) XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__),  \
                                            (__VA_ARGS__), __VA_ARGS__)

#define xmessage2_define(name, ...)                                      \
  XMessage name;                                                         \
  name XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__), (__VA_ARGS__), \
                             __VA_ARGS__)
#define xmessage2(...)                                                         \
  XMessage() XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__), (__VA_ARGS__), \
                                   __VA_ARGS__)

#define XLOGGER_SCOPE_MESSAGE(...)                                           \
  PP_IF(PP_NUM_PARAMS(__VA_ARGS__), xmessage2(__VA_ARGS__).String().c_str(), \
        NULL)
#define __xscope_impl(level, name, ...)                             \
  XScopeTracer __ANONYMOUS_VARIABLE__(_tracer_)(                    \
      level, XLOGGER_TAG, name, __XFILE__, __XFUNCTION__, __LINE__, \
      XLOGGER_SCOPE_MESSAGE(__VA_ARGS__))

#define xverbose_scope(name, ...) \
  __xscope_impl(kLevelVerbose, name, __VA_ARGS__)
...
#define __xfunction_scope_impl(level, name, ...)                    \
  XScopeTracer ____xloger_anonymous_function_scope_20151022____(    \
      level, XLOGGER_TAG, name, __XFILE__, __XFUNCTION__, __LINE__, \
      XLOGGER_SCOPE_MESSAGE(__VA_ARGS__))

#define xverbose_function(...) \
  __xfunction_scope_impl(kLevelVerbose, __FUNCTION__, __VA_ARGS__)
#define xdebug_function(...) \
  __xfunction_scope_impl(kLevelDebug, __FUNCTION__, __VA_ARGS__)
#define xinfo_function(...) \
  __xfunction_scope_impl(kLevelInfo, __FUNCTION__, __VA_ARGS__)
#define xexitmsg_function(...)                           \
  ____xloger_anonymous_function_scope_20151022____.Exit( \
      xmessage2(__VA_ARGS__).String())
